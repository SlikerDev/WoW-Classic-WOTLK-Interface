...
Talented.importers["/%?pet-talent-calc#"] = function (self, url, dst)
	local s, _, code = url:find(".*/%?pet-talent-calc#(.*)$")
	if not s or not code then return end
	code = ImportCode(code)
	if not code then return end
	local res, val, class = pcall(self.StringToTemplate, self, code, dst)
	if not res then
		return
	end
	dst.class = class
...
function Talented:ExportWhpetTemplate(template)
	...
	if code then
		return L["https://www.wowhead.com/wotlk/pet-talent-calc/%s/%s"]:format(code)
	end
end