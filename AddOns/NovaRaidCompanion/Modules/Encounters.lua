...
--[[hooksecurefunc("StaticPopup_Show", function(...)
	for i = 1, STATICPOPUP_NUMDIALOGS do
		...
		if (frame.which == "RESURRECT_NO_SICKNESS" and _G["StaticPopup" .. i]:IsShown()) then
			if (not frame.nrcHook) then
				frame:HookScript("OnUpdate", function(self)
					if (frame.which == "RESURRECT_NO_SICKNESS") then
				...
				if (itemSubType == META_GEM) then
					...
					for line = 2, 4 do
						...
						if (lineText and lineText ~= ITEM_BIND_ON_PICKUP and lineText ~= ITEM_UNIQUE_EQUIPPABLE
								 and lineText ~= ITEM_QUALITY2_DESC and lineText ~= ITEM_QUALITY3_DESC and lineText ~= ITEM_QUALITY4_DESC) then
							gemEffect = lineText;
							break;
						end
					end
